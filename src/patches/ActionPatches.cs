...
namespace H3MP.Patches
{
    ...
    class RoundPatch
    {
        ...
        static bool SplodePrefix(FVRFireArmRound __instance, float velMultiplier, bool isRandomDir)
        {
            ...
            if(trackedItem != null)
            {
                // A non controller should never Splode a round unless ordered to by controller
                if (trackedItem.data.controller == GameManager.ID)
                {
                    if (ThreadManager.host)
                    {
                ...
                {
                    // Instead of patching OnParticleCollision, I decided to let it call Splode and just reset isCookingOff to false
                    // before preventing Splode from running
                    __instance.isCookingOff = false;
                    return false;
                }
            }
...
if (__instance.IsSuppressed())
{
    ...
    {
        if (player.playerBody != null && player.playerBody.PlayerEntities != null && player.playerBody.PlayerEntities.Count > 0)
        {
            GM.CurrentSceneSettings.OnPerceiveableSound(__instance.AudioClipSet.Loudness_Suppressed,
                ...
                pos, IFF, player.playerBody.PlayerEntities[0]);
        }
    }
